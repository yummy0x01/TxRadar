...
fn main() -> anyhow::Result<()> {
    // TxRadar local patch: upstream 6.1.0 sets PROTOC via `protobuf-src`, which
    // compiles the protobuf C++ compiler from source through autotools. That
    // cannot work on Windows/MSVC (autotools can't drive cl.exe). We instead use
    // `protoc-bin-vendored`, which ships a prebuilt protoc binary per platform —
    // a pure download, no C++ toolchain. This is the same approach upstream
    // adopted in later (agave-3/4) releases.
    std::env::set_var(
        "PROTOC",
